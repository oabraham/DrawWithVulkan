...
private:
    ...
    static std::vector<char> readFile(const std::string& filename) {
        ...
        //get file size by querying the last character in the file
        size_t fileSize = (size_t) file.tellg();
        std::vector<char> buffer(fileSize); //create a space to store the characters

        file.seekg(0); // go to the first character inf ile
        file.read(buffer.data(), fileSize); //read the file at once
